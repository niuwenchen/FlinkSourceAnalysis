...
import com.jackniu.flink.api.java.typeutils.runtime.RowComparator;
import com.jackniu.flink.api.java.typeutils.runtime.RowSerializer;
...
public class RowTypeInfo extends TupleTypeInfoBase<Row> {
	...
	public void getFlatFields(String fieldExpression, int offset, List<FlatFieldDescriptor> result) {
		...
		if ((field.equals(Keys.ExpressionKeys.SELECT_ALL_CHAR)) ||
			(field.equals(Keys.ExpressionKeys.SELECT_ALL_CHAR_SCALA))) {
			// handle select all
			int keyPosition = 0;
			for (TypeInformation<?> fType : types) {
				if (fType instanceof CompositeType) {
					CompositeType<?> cType = (CompositeType<?>) fType;
					cType.getFlatFields(Keys.ExpressionKeys.SELECT_ALL_CHAR, offset + keyPosition, result);
